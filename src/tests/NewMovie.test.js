...
describe('6 - Insira um link na página inicial para `NewMovie` para criar novos cartões', () => {
  ...
  test('Será validado se a página inicial contém um link "ADICIONAR CARTÃO". Esse link deve redirecionar para a página de criação de filmes', async () => {
  ...
  test('Será validado se `NewMovie` contém um formulário que faz uma requisição para API para criar um novo filme. Após a criação, a pessoa usuária deverá ser redirecionada para a página inicial', async () => {
    cleanup();
