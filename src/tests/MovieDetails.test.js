...
describe('4 - Faça uma requisição para buscar o filme que deverá ser renderizado dentro de `Movie Details`', () => {
  ...
  test('Será validado se `MovieDetails` exibe o texto "Carregando..." enquanto estiver fazendo a requisição', async () => {
  ...
  test('Será validado se `MovieDetails` contém um botão com o texto "VOLTAR" que redireciona para a página inicial', async () => {
    for (const movie of readMovies()) {
  ...
  test('Será validado se `MovieDetails` contém um botão com o texto "EDITAR" que redireciona para a página de edição de filme', async () => {
    for (const movie of readMovies()) {
      const { unmount, findByText } = renderPath('/movies/' + movie.id);
      await waitFor(() => movieAPI.getMovie(movie.id));
