...
describe('5 - Realize uma requisição para buscar o filme que será editado em `EditMovie`', () => {
  ...
  test.skip('Será validado se `EditMovie` exibe o texto "Carregando..." enquanto estiver fazendo a requisição', async () => {
  ...
  test.skip('Será validado se `EditMovie` contém um formulário preenchido com o título, subtítulo, sinopse, caminho da imagem e gênero do filme selecionado', async () => {
    for (const movie of readMovies()) {
  ...
  test.skip('Será validado se, ao clicar no botão de submit, uma requisição para API é feita e o filme selecionado é atualizado. Após a conclusão da atualização a pessoa usuária deve ser redirecionada para a página inicial', async () => {
    for (const movie of readMovies()) {
      cleanup();
